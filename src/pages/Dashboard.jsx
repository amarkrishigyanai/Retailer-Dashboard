      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p
            ...
            style={{ fontWeight: 800, color: HEADING }}
          >
            {value}
          </p>
        </div>
...
function Dashboard() {
  ...
  ledgerEntries.forEach((e) => {
    ...
    if (!customerBalanceMap[id])
      customerBalanceMap[id] = { user: e.user, net: 0 };
    customerBalanceMap[id].net +=
      e.type === "CREDIT" ? Number(e.amount || 0) : -Number(e.amount || 0);
  });
  ...
  return (
    <div className="space-y-6">
      {/* HEADER */}
      <div>
        <h1 className="text-2xl" style={{ fontWeight: 800, color: HEADING }}>
          Dashboard Overview
        </h1>
        <p className="text-sm" style={{ color: MUTED }}>
          Welcome back! Here's what's happening with your {theme.orgName} today.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          value={`₹${Number(revenueOrdersChart.at(-1)?.revenue ?? 0).toLocaleString("en-IN")}`}
          icon={TrendingUp}
          path="/ledger"
          iconBg="#C6EFD9"
          iconColor="#2BB57A"
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            >
              <div className="mb-4">
                ...
                <p className="text-xs mt-0.5" style={{ color: MUTED }}>
                  Revenue earned from orders over the last 6 months
                </p>
              </div>
              {/* Line Chart */}
              <ResponsiveContainer width="100%" height={144}>
                <AreaChart
                  data={revenueOrdersChart}
                ...
                >
                  ...
                  <Tooltip
                    ...
                    labelFormatter={(label) => {
                      const d = revenueOrdersChart.find(
                        (r) => r.month === label,
                      );
              ...
              >
                <div className="flex-1 text-center">
                  ...
                  <p className="text-sm font-bold" style={{ color: HEADING }}>
                    ₹
                    {Number(lastActiveMonth?.revenue ?? 0).toLocaleString(
                      "en-IN",
                    )}
                  </p>
                </div>
                ...
                <div className="flex-1 text-center">
                  ...
                  <p className="text-sm font-bold" style={{ color: HEADING }}>
                    ...
                    {Number(
                      revenueOrdersChart[
                        revenueOrdersChart.indexOf(lastActiveMonth) - 1
                      ]?.revenue ?? 0,
        ...
        {(() => {
          const totalProcurement = procVsSalesChart.reduce(
            (s, d) => s + d.procurement,
            0,
          );
          const totalSales = procVsSalesChart.reduce((s, d) => s + d.sales, 0);
          const bestProcMonth = procVsSalesChart.reduce(
            (a, b) =>
              b.sales - b.procurement > a.sales - a.procurement ? b : a,
            procVsSalesChart[0] ?? {},
          );
          const lastActiveProcMonth =
            [...procVsSalesChart]
              .reverse()
              .find((d) => d.procurement > 0 || d.sales > 0) ??
            procVsSalesChart.at(-1);
          return (
            <div
              className="p-6 rounded-2xl"
              style={{
                  ...
                  <div className="flex items-end gap-3">
                    <div>
                      ...
                      <p
                        ...
                        style={{ color: HEADING }}
                      >
                        ₹{Number(totalProcurement).toLocaleString("en-IN")}
                      </p>
                    </div>
                    <div className="pb-0.5" style={{ color: MUTED }}>
                      vs
                    </div>
                    <div>
                      <p
              ...
              >
                ...
                <div className="flex-1 text-center">
                  ...
                  <p
                    ...
                    style={{
                      color:
                        totalSales - totalProcurement >= 0 ? SAGE : "#ef4444",
                    }}
                  >
                    {totalSales - totalProcurement >= 0 ? "+" : ""}₹
                    {Number(
                      Math.abs(totalSales - totalProcurement),
                    ).toLocaleString("en-IN")}
                  </p>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        {(() => {
          ...
          const growthPct =
            prev > 0 ? (((latest - prev) / prev) * 100).toFixed(1) : null;
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs mt-0.5" style={{ color: MUTED }}>
                    Total customers · {memberGrowthChart.at(-1)?.month}
                  </p>
                </div>
                <div className="flex flex-col items-end gap-1.5">
                  {growthPct !== null && (
                    <div
                      ...
                      style={{
                        backgroundColor: isGrowing ? SAGE_100 : "#FEE2E2",
                        color: isGrowing ? SAGE : "#EF4444",
                      }}
                    >
                      {isGrowing ? "▲" : "▼"} {Math.abs(growthPct)}% vs last
                      month
                    </div>
                  )}
                  <div
                ...
                >
                  ...
                  <Tooltip
                    ...
                    formatter={(val, name, props) => {
                      const idx = memberGrowthChart.findIndex(
                        (d) =>
                          d.total === val && d.month === props.payload.month,
                      );
                      const diff =
                        idx > 0 ? val - memberGrowthChart[idx - 1].total : 0;
                      return [
                        `${val} members${diff > 0 ? ` (+${diff} new)` : ""}`,
                        "Total",
                      ];
                    }}
                  />
              ...
              >
                ...
                <div className="flex-1 text-center">
                  ...
                  <p
                    className="text-sm font-bold"
                    style={{ color: isGrowing ? SAGE : "#EF4444" }}
                  >
                    {isGrowing ? "+" : ""}
                    {added}
                  </p>
        ...
        {(() => {
          const maxStock = inventoryStockChart.length
            ? Math.max(...inventoryStockChart.map((p) => p.stock || 0))
            : 1;
          const lowStockProducts = inventoryStockChart.filter(
            (p) => p.stock <= p.minStock,
          );
          const adequateStock = inventoryStockChart.filter(
            (p) => p.stock > p.minStock,
          );
            ...
            >
              <div className="mb-6">
                ...
                <p className="text-xs" style={{ color: MUTED }}>
                  Current stock status across products
                </p>
              </div>

              {inventoryStockChart.length === 0 ? (
                <div
                  className="flex flex-col items-center justify-center h-[220px]"
                  style={{ color: MUTED }}
                >
      ...
      {(() => {
        const activeProducts =
          inventoryStockChart.length > 0 ? inventoryStockChart : [];
        const topActive =
          allListings
        ...
        return topActive.length === 0 ? null : (
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <div>
                <h3 className="font-bold mb-0.5" style={{ color: HEADING }}>
                  🛍️ Active Product Listings
                </h3>
                <p className="text-xs" style={{ color: MUTED }}>
                  Recently active products in your catalog
                </p>
              </div>
              <button
                ...
                <thead>
                  <tr style={{ borderBottom: "2px solid #f0f4f8" }}>
                    {[
                      ...
                      <th
                        key={h}
                        className="pb-3 text-left text-[11px] font-bold uppercase tracking-wide"
                        style={{ color: MUTED }}
                      >
                        {h}
                      </th>
                          ...
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              ...
                              <p
                                ...
                                style={{ color: MUTED }}
                              >
                                {p.addedBy?.firstName} {p.addedBy?.lastName}
                              </p>
                            </div>
                          </div>
                        </td>
        ...
        >
          <div className="flex items-center justify-between mb-1">
            ...
            <span
              ...
              style={{ backgroundColor: "#ECFDF5", color: "#34D399" }}
            >
              {topCropsChart.reduce((s, d) => s + d.value, 0)} Orders
            </span>
          </div>
          <p className="text-xs mb-4" style={{ color: MUTED }}>
            Breakdown of marketplace orders by status
          </p>
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      style={{
                        backgroundColor: isCredit ? "#FEF2F2" : SAGE_100,
                      }}
                    >
                      {isCredit ? (
                        <TrendingDown
                    ...
                    <div className="text-right flex-shrink-0">
                      <p
                        className="text-sm font-bold"
                        style={{ color: isCredit ? "#F87171" : SAGE }}
                      >
                        {isCredit ? "-" : "+"}
                        {fmtClean(e.amount)}
                      </p>
      ...
      {ledgerEntries.length > 0 && (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-3">
            {[
              {
                label: "To Pay Customers",
                sub: "Pending payments",
                value: fmtClean(Math.max(0, totalToPayCustomers)),
                icon: TrendingDown,
                iconBg: "#fee2e2",
                iconColor: "#ef4444",
                valColor: "#ef4444",
              },
              {
                ...
                value: fmtClean(Math.max(0, totalToCollect)),
                icon: TrendingUp,
                iconBg: SAGE_100,
                iconColor: SAGE,
                valColor: SAGE,
              },
              {
                label: "Net Balance",
                sub:
                  Math.max(0, totalToPayCustomers) >=
                  Math.max(0, totalToCollect)
                    ? `${theme.orgName} owes customers`
                    : `Customers owe ${theme.orgName}`,
                value: fmtClean(
                  Math.abs(
                    Math.max(0, totalToPayCustomers) -
                      Math.max(0, totalToCollect),
              ...
              <div
                key={item.label}
                className="p-5 flex items-center gap-4 rounded-2xl"
                style={{
                  backgroundColor: CARD_BG,
                  border: "1px solid rgba(0,0,0,0.06)",
                }}
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <div
                        ...
                        style={{ backgroundColor: "#C6EFD9", color: HEADING }}
                      >
                    ...
                    <div className="text-right">
                      <p
                        className="text-sm font-bold"
                        style={{ color: r.net > 0 ? "#ef4444" : SAGE }}
                      >
                        {fmtClean(Math.abs(r.net))}
                      </p>
                      <p
                        className="text-[10px] font-semibold"
                        style={{ color: r.net > 0 ? "#ef4444" : "#2BB57A" }}
                      >
                        {r.net > 0 ? `${theme.orgName} owes` : "Customer owes"}
                      </p>
                    </div>
                  </div>
                ))}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        >
          <h3 className="mb-2 font-bold" style={{ color: HEADING }}>
            Quick Actions
          </h3>
          <button
            ...
            onMouseEnter={(e) =>
              (e.currentTarget.style.backgroundColor = "#C6EFD9")
            }
            onMouseLeave={(e) =>
          ...
          <button
            ...
            onMouseEnter={(e) =>
              (e.currentTarget.style.backgroundColor = "#E8F8F1")
            }
            onMouseLeave={(e) =>
              (e.currentTarget.style.backgroundColor = "transparent")
            }
