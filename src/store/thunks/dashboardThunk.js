...
export const getDashboardData = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const [listingsRes, ordersRes, membersRes, purchasesRes, inventoryRes] =
        await Promise.all([
      ...
      const approvedListings = products.filter(
      ...
      const pendingDeliveries = Array.isArray(orders)
        ? orders.filter((o) =>
            ["pending", "approved"].includes(normalizeStatus(o.status)),
          ).length
        : 0;
      ...
      const topCropsChart = [
        {
          name: "Pending",
          value: obStatusCount["PENDING"],
          color: "#FBBF24",
          bg: "#FFFBEB",
        },
        {
          name: "Approved",
          value: obStatusCount["APPROVED"],
          color: "#60A5FA",
          bg: "#EFF6FF",
        },
        {
          name: "Sold",
          value: obStatusCount["SOLD"],
          color: "#34D399",
          bg: "#ECFDF5",
        },
        {
          name: "Cancelled",
          value: obStatusCount["CANCELLED"],
          color: "#F87171",
          bg: "#FEF2F2",
        },
      ].filter((d) => d.value > 0);
      ...
      const revenueOrdersChart = monthLabels.map(({ key, label }) => {
        ...
        const revenue = Array.isArray(orders)
          ? orders
              ...
              .reduce(
                (s, o) => s + Number(o.finalAmount || o.totalAmount || 0),
                0,
              )
          : 0;
      ...
      const procVsSalesChart = monthLabels.map(({ key, label }) => {
        ...
        const procValue = purchases
          ...
          .reduce((s, p) => {
            const fromCrops =
              p.crops?.reduce(
                (cs, c) => cs + Number(c.rate || 0) * Number(c.quantity || 0),
                0,
              ) ?? 0;
        ...
        const salesValue = Array.isArray(orders)
          ? orders
              ...
              .reduce(
                (s, o) =>
                  s + Number(o.finalAmount || o.totalAmount || o.amount || 0),
                0,
              )
          : 0;
      ...
      if (Object.keys(productSalesMap).length === 0) {
        purchases.forEach((p) => {
          ...
          if (purchaseYear === currentYear && purchaseMonth === currentMonth) {
            const name =
              p.crop || p.crops?.[0]?.cropName || p.productName || "Unknown";
        ...
        .map(([productId, data]) => {
          ...
          if (foundProduct) {
            return {
              _id: productId,
              name:
                foundProduct.productName || foundProduct.name || data.itemName,
              qty: data.qty,
              revenue: data.revenue,
              image: foundProduct.productImages?.[0]?.url || foundProduct.image,
              price: Number(
                foundProduct.price || foundProduct.sellingPrice || 0,
              ),
            };
          }
          ...
          if (data.itemName) {
            return {
              ...
              revenue: data.revenue,
              image:
                data.product?.productImages?.[0]?.url || data.product?.image,
              price: Number(
                data.product?.price || data.product?.sellingPrice || 0,
              ),
            };
          }
      ...
      purchases.forEach((p) => {
        ...
        if (name)
          purchasedQtyMap[name] =
            (purchasedQtyMap[name] || 0) +
            Number(p.quantity || p.crops?.[0]?.quantity || 0);
      });
